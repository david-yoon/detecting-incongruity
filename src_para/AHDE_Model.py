...
class AttnHrDualEncoderModel:
    ...
    def __init__(self, voca_size, batch_size,
        ...
        self.dr_text_in   = Params.dr_text_in
        self.dr_text_out = Params.dr_text_out
        self.dr_con_in   = Params.dr_con_in
        self.dr_con_out = Params.dr_con_out
        
        self.memory_dim = memory_dim
        self.topic_size = topic_size
    
        # create in the build model
        self.encoder_inputs = []
        self.context_inputs = []
    ...
    """
    barch          : batch size
    encoding_length: how many turns of utterance will be encoded
    time_step      : how many sequences of a word will be considered in each utterance
    """
    ...
    def build_graph(self):
        ...
        if Params.add_attention : self._create_attention_layers()
...
def train_model(model, batch_gen, num_train_steps, valid_freq, is_save=0, graph_dir_name='default'):
    ...
    with tf.Session(config=config) as sess:
        ...
        for index in xrange(num_train_steps):
            ...
            if (model.global_step.eval() * model.batch_size / float(len(batch_gen.train_set))) > Params.num_till_max_epoch:
                print "reach max epoch: ", Params.num_till_max_epoch
                break
...
def main(batch_size, encoder_size, context_size, encoderR_size, num_layer, hidden_dim, num_layer_con, hidden_dim_con,
    ...
    model = AttnHrDualEncoderModel(voca_size=len(batch_gen.voca),
