...
class ProcessData:
    ...
    def load_data(self):
        ...
        if Params.IS_DEBUG :
            ...
            self.test_data['id'], self.test_data['c'], self.test_data['r'] = pickle.load( open( Params.DATA_DIR + 'para/' + Params.DATA_DEBUG_TITLE_BODY, 'r') )
            self.test_data['y'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_TEST_LABEL)
            ...
            self.test_data['id'], self.test_data['c'], self.test_data['r'] = pickle.load( open( Params.DATA_DIR + 'para/' + Params.DATA_TEST_TITLE_BODY, 'r') )
            self.test_data['y'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_TEST_LABEL)
            ...
            self.train_data['c'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_TRAIN_TITLE)
            self.train_data['r'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_TRAIN_BODY)
            self.train_data['y'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_TRAIN_LABEL)

            self.valid_data['c'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_DEV_TITLE)
            self.valid_data['r'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_DEV_BODY)
            self.valid_data['y'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_DEV_LABEL)
            
            self.test_data['id'], self.test_data['c'], self.test_data['r'] = pickle.load( open( Params.DATA_DIR + 'para/' + Params.DATA_TEST_TITLE_BODY, 'r') )
            self.test_data['y'] = np.load(Params.DATA_DIR + 'para/' + Params.DATA_TEST_LABEL)
                    
        self.voca = pickle.load(open(Params.DATA_DIR + 'para/' + Params.VOCA_FILE_NAME, 'r') )
    ...
    # create train set : 
    # source_ids : 'c' (question)  
    # target_ids : 'r' (answer) -> split by using '<EOS>' -> multiple sentneces
    ...
    def create_data_set(self, input_data, output_set, set_type):
        ...
        for index in tqdm(xrange(data_len)):
            ...
            if (set_type != 'TEST') :
                ...
                # test set use different label ( len(data) != len(label) because the text was split into several sentneces in para-method )
                if index > len(input_data['y'])-1 : label = float(input_data['y'][-1])
                else                              : label = float(input_data['y'][index])
